...
const potsTimeline = [...document.querySelectorAll(".pot")].map((pot, i) => {
    ...
    timeline.from(pot, POTS_TIMES[i].duration, {
...
const shadowsTimeline = [...document.querySelectorAll(".pot-shadow")].map(
    (shadow, i) => {
        ...
        timeline.from(shadow, POTS_TIMES[i].duration, {
...
const leafsTimeline = [...document.querySelectorAll(".leaf")].map((leaf) => {
    ...
    timeline.from(leaf, 0.5 + Math.random() * 0.8, {
        delay: isBack ? 0.5 : 0,
...
const flowersTimeline = [...document.querySelectorAll(".flower")].map(
    (flower) => {
        ...
        timeline.from(flower, 1 + Math.random() * 1, {
            ease: Circ.easeOut,
            scale: 0,
